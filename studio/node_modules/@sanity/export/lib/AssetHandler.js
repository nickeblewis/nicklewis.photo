  ...
  downloadAsset(assetDoc, dstPath) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (differs && attemptNum < 3) {
        ...
        var details = [method === 'md5' ? "md5 should be ".concat(remoteMd5, ", got ").concat(md5) : "sha1 should be ".concat(remoteSha1, ", got ").concat(sha1), contentLength && parseInt(contentLength, 10) !== size && "Image should be ".concat(contentLength, " bytes, got ").concat(size), "Did not succeed after ".concat(attemptNum, " attempts.")];
        var detailsString = "Details:\n - ".concat(details.filter(Boolean).join('\n - '));
        yield fse.unlink(tmpPath);

        _this2.queue.clear();

        var error = new Error("Failed to download image at ".concat(assetDoc.url, ", giving up. ").concat(detailsString));
