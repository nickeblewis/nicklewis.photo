...
var valid = (allowedValues, actual, message) => {
  ...
  var value = (valueType === 'number' || valueType === 'string') && `${actual}`;
  var strValue = value && value.length > 30 ? `${value.slice(0, 30)}…` : value;
  var defaultMessage = value ? `Value "${strValue}" did not match any of allowed values` : 'Value did not match any of allowed values';
...
function () {
  var _ref = _asyncToGenerator(function* (fn, value, message, options) {
    ...
    try {
      ...
      err.message = `${_path}: Error validating value: ${err.message}`;
      throw err;
    }
