...
var dataAspects = new _resolver.default(_schema.default);
...
var _default = (0, _router.withRouterHOC)((_temp = _class = class DefaultLayout extends _react.default.Component {
  constructor() {
    ...
    _defineProperty(this, "renderContent", () => {
      ...
      var TYPE_ITEMS = dataAspects.getInferredTypes().filter(typeName => (0, _documentActionUtils.isActionEnabled)(_schema.default.get(typeName), 'create')).map(typeName => ({
        key: typeName,
        name: typeName,
        title: dataAspects.getDisplayName(typeName),
        icon: dataAspects.getIcon(typeName)
      }));
      var modalActions = TYPE_ITEMS.map(item => ({
        title: item.title,
        icon: item.icon,
        params: {
          type: item.name
