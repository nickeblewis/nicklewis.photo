...
class DeskToolPanes extends _react.default.Component {
  ...
  renderPanes() {
    ...
    return panes.map((pane, i) => {
      ...
      var paneKey = `${i}-${keys[i - 1] || 'root'}`; // Same pane might appear multiple times, so use index as tiebreaker

      var wrapperKey = pane === _resolvePanes.LOADING ? `loading-${i}` : `${i}-${pane.id}`;
      path.push(pane.id || `[${i}]`);
