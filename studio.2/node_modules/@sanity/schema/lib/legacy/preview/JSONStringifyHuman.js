...
function _stringify(value, options, depth) {
  ...
  if (Array.isArray(value)) {
    ...
    var _asString = value.slice(0, options.maxBreadth).map((item, index) => _stringify(item, options, depth + 1)).concat(capLength > 0 ? `…+${capLength}` : []).join(', ');
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    var _asString2 = keys.slice(0, options.maxBreadth).map(key => `${key}: ${_stringify(value[key], options, depth + 1)}`).join(', ');
