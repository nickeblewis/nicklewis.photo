...
function generateCommandsDocumentation(commandGroups) {
  ...
  var rows = ['usage: sanity [-v|--version] [-d|--debug] [-h|--help] <command> [<args>]', '', 'Commands:'].concat(commandGroup.map(cmd => `   ${(0, _padEnd2.default)(cmd.name, cmdLength)} ${cmd.description}`)).concat(['', "See 'sanity help <command>' for specific information on a subcommand."]);
  return rows.join('\n');
}
/**
