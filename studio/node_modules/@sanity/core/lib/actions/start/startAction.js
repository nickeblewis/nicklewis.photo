...
function gracefulDeath(httpHost, config, err) {
  ...
  if (err.code === 'EACCES') {
    var help = config.port < 1024 ? 'port numbers below 1024 requires root privileges' : `do you have access to listen to the given host (${httpHost})?`;
...
function printErrors(output, errors) {
  ...
  output.print('');
  var formattedErrors = (errors.some(_formatMessage.isLikelyASyntaxError) ? errors.filter(_formatMessage.isLikelyASyntaxError) : errors).map(message => `Error in ${(0, _formatMessage.formatMessage)(message)}`);
