...
class ValidationStatus extends _react.default.PureComponent {
  render() {
    ...
    var errorDef = `error${errors.length === 1 ? '' : 's'}`;
    var errorText = errors.length > 0 && `${errors.length} ${errorDef}`;
    var warningDef = `warning${warnings.length === 1 ? '' : 's'}`;
    var warningText = warnings.length > 0 && `${warnings.length} ${warningDef}`;
