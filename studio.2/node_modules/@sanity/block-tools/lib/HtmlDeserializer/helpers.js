...
function createRuleOptions(blockContentType) {
  var options = (0, _blockContentTypeToOptions.default)(blockContentType);

  var mapItem = item => item.value;

  var enabledBlockStyles = options.styles.map(mapItem);
  var enabledSpanDecorators = options.decorators.map(mapItem);
  var enabledBlockAnnotations = options.annotations.map(mapItem);
  return {
    enabledBlockStyles,
